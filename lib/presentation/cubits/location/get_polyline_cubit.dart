...
class GetPolylineCubit extends Cubit<GetPolylineState> {
  ...
  Future<void> getPolyline({
    ...
    try {
      ...
      final routes = await GoogleRoutesService.computeRoutes(
        origin: RouteCoordinate(sourcelat, sourcelng),
        destination: RouteCoordinate(destinationlat, destinationlng),
      );

      if (routes.isNotEmpty && routes.first.encodedPolyline != null) {
        final polylineCoordinates = GoogleRoutesService.decodePolyline(
        ...
        emit(GetPolylineUpdatedError("Failed to get polyline"));
      }
    } catch (e) {
