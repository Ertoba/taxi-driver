...
document.addEventListener('resume', function(){ //fires when app is pulled up from background
    ...
    if(map)updateDriverLocation();
    setTimeout(function(){
        animateDriversMarkers();
    },5000);

    backgroundGeoloacationStart(); //check and ensure background location is always running
                ...
                document.querySelector('#myNavigator').popPage(
                    {
                        ...
                        times : 3,
                        callback : function(){
                            $('#country-flag').attr('class', 'iti__flag iti__' + driver_registration_data.country_2c_code);
                            $('#country-flag').data('country', driver_registration_data.country_2c_code)
                            $('#tel-code').html(' +' + driver_registration_data.country_call_code);
                            $('#tel-code').data('dialcode', driver_registration_data.country_call_code);
                            $('#login-phone').val(driver_registration_data.phone);
                            $('#login-password').val(driver_registration_data.password);
                            $('#loginbut').click();
                            return; 
                        }   
                    }
...
async function finalize_init(){
    ...
    BackgroundGeolocation.configure({
        url: ajaxurl + "&action_get=setDriverLocationbg",
        /* interval: parseInt(app_settings.driver_location_update_interval) > 5 ? parseInt(app_settings.driver_location_update_interval) * 1000 : 5000,
        fastestInterval: parseInt(app_settings.driver_location_update_interval) > 5 ? parseInt(app_settings.driver_location_update_interval) * 1000 : 5000, */
        httpHeaders: {
          'X-DRIVERLOCATION': Date.now()
        },
        // customize post properties
        postTemplate: {
            action: 'setDriverLocationbg',
            lat: '@latitude',
            long: '@longitude'
        }
    });
